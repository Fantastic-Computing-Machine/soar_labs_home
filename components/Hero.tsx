...
const Hero: React.FC = () => {
  ...
  const dynamicY1 = useTransform(scrollY, [0, 1000], [0, 200]);
  const dynamicY2 = useTransform(scrollY, [0, 1000], [0, -150]);
  const dynamicY3 = useTransform(scrollY, [0, 1000], [0, 100]);
  const y1 = prefersReducedMotion ? staticOffset : dynamicY1;
  const y2 = prefersReducedMotion ? staticOffset : dynamicY2;
  const y3 = prefersReducedMotion ? staticOffset : dynamicY3;
  ...
  return (
    <section className="relative pt-32 pb-20 lg:pt-48 lg:pb-32 overflow-hidden">
      ...
      <div className="absolute inset-0 -z-10 overflow-hidden pointer-events-none">
        ...
        <motion.div style={{ y: y1 }} className="absolute top-[-10%] left-[-10%] w-[50%] h-[50%]">
          <div className="w-full h-full rounded-full bg-purple-100/40 blur-[100px] animate-blob mix-blend-multiply filter"></div>
        </motion.div>
        
        <motion.div style={{ y: y2 }} className="absolute top-[-10%] right-[-10%] w-[50%] h-[50%]">
        ...
        <motion.div style={{ y: y3 }} className="absolute top-[20%] left-[20%] w-[40%] h-[40%]">
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          ...
          >
            ...
            <p className="text-lg text-slate-600 mb-8 max-w-xl leading-relaxed">
              Soar Labs is the enterprise-ready RAG platform that connects your data to LLMs in milliseconds. Secure, scalable, and effortlessly simple.
            </p>
            
            <div className="flex flex-col sm:flex-row gap-4">
              <button className="bg-slate-900 hover:bg-slate-800 text-white px-8 py-4 rounded-xl font-semibold text-lg transition-all flex items-center justify-center gap-2 shadow-xl shadow-slate-900/20 hover:shadow-slate-900/30 hover:scale-[1.02] active:scale-[0.98]">
